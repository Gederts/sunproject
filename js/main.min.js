const siteName="Sun Project",email="info@sunproject.lv",HeaderComponent={template:`
    ...
    </div>`,props:{data:{type:Array,required:!0}},data:()=>({page:0}),async mounted(){await this.$nextTick();let _=window.location.hash.replace("#","");this.data.forEach(($,t)=>{if($.toLowerCase().replace(" ","-")===_){this.page=t,this.$root.$emit("switch",t);return}})},methods:{switchContent(_){this.page=_,this.$root.$emit("switch",_),window.location.hash="#"+this.data[_].toLowerCase().replace(" ","-"),window.scrollTo({top:0,behavior:"smooth"})}}},ContentComponent={template:`
    <div id="content">
        ...
        `,data:()=>({page:0,section:0}),mounted(){this.$root.$on("switch",_=>{this.page=_})},methods:{switchSection(_){this.section=_}}},FooterComponent={template:`
    <div class="bg-light">
        <hr>
        ...
        `},app=new Vue({el:"#app",components:{"header-component":HeaderComponent,"content-component":ContentComponent,"footer-component":FooterComponent}});